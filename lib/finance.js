...
/** Analyze a chronological metric series: level, direction, volatility. */
export function analyzeTrend({ values = [], labels = [] } = {}) {
  const list = (Array.isArray(values) ? values : []).map(Number).filter((x) => Number.isFinite(x));
  if (list.length < 2) throw new Error("finance: trend needs at least two numeric values");
  const n = list.length;
  ...
  let num = 0;
  let den = 0;
  for (let i = 0; i < n; i++) { num += (i - xMean) * (list[i] - mean); den += (i - xMean) ** 2; }
  const slope = den !== 0 ? num / den : 0;
  const stdev = Math.sqrt(list.reduce((a, b) => a + (b - mean) ** 2, 0) / n);
  const changes = [];
  for (let i = 1; i < n; i++) changes.push(Math.abs(list[i] - list[i - 1]));
  const avgAbsChange = changes.reduce((a, b) => a + b, 0) / changes.length;
  const direction = slope > 0 ? "up" : slope < 0 ? "down" : "flat";
  const text = [
    "# Trend analysis",
    "",
    "n=" + n + " | mean=" + round2(mean) + " | first=" + first + " | last=" + last,
    "change=" + round2(change) + (changePct === null ? "" : " (" + round2(changePct) + "%)") + " | direction=" + direction,
    "min=" + min + " (idx " + list.indexOf(min) + ") | max=" + max + " (idx " + list.indexOf(max) + ")",
    "slope=" + round2(slope) + " | stdev=" + round2(stdev) + " | avg abs change=" + round2(avgAbsChange),
    "",
    "| i | label | value |",
    "|---|-------|-------|",
    ...list.map((v, i) => "| " + i + " | " + (labels[i] !== undefined ? labels[i] : i) + " | " + v + " |"),
  ].join("\n");
  return { n, mean: round2(mean), first, last, change: round2(change), changePct: changePct === null ? null : round2(changePct), min, max, slope: round2(slope), stdev: round2(stdev), avgAbsChange: round2(avgAbsChange), direction, text };
}

/** Compare one value against a peer distribution (P10/P50/P90 + percentile rank). */
export function peerCompare({ value = 0, peers = [], metric = "metric", higherIsBetter = true } = {}) {
  const v = Number(value);
  if (!Number.isFinite(v)) throw new Error("finance: value must be a finite number");
  const list = (Array.isArray(peers) ? peers : []).map(Number).filter((x) => Number.isFinite(x));
  if (list.length === 0) throw new Error("finance: peers must contain at least one number");
  const sorted = [...list].sort((a, b) => a - b);
  const p10 = round2(percentile(sorted, 0.1));
  const p50 = round2(percentile(sorted, 0.5));
  const p90 = round2(percentile(sorted, 0.9));
  const below = list.filter((x) => x < v).length;
  const rank = round2((below / list.length) * 100);
  const position = rank >= 75 ? "top quartile" : rank >= 50 ? "above median" : rank >= 25 ? "below median" : "bottom quartile";
  const favorable = higherIsBetter ? rank >= 50 : rank < 50;
  const text = [
    "# Peer comparison: " + metric,
    "",
    "value=" + v + " | peers=" + list.length + " | percentile rank=" + rank + " (" + position + ")",
    "P10=" + p10 + " | P50=" + p50 + " | P90=" + p90,
    "vs median: " + round2(v - p50) + (higherIsBetter ? " (higher is better)" : " (lower is better)"),
    "verdict: " + (favorable ? "favorable" : "unfavorable"),
  ].join("\n");
  return { value: v, n: list.length, p10, p50, p90, rank, position, favorable, text };
}